...
def mostrar_metricas_cv(modelo, X, y, cv=5):
    """
    Muestra métricas y gráficos utilizando predicciones de cross-validation.
    """
    # Realizar predicciones con Cross-Validation
    y_pred = cross_val_predict(modelo, X, y, cv=cv)
    
    # Calcular métricas
    precision = accuracy_score(y, y_pred) * 100
    f1 = f1_score(y, y_pred, average='micro') * 100
    recall = recall_score(y, y_pred, average='weighted') * 100
    report = classification_report(y, y_pred)
    cm = confusion_matrix(y, y_pred)
    
    # Crear figura
    fig, ax = plt.subplots(1, 2, figsize=(12, 5), gridspec_kw={'width_ratios': [1, 2]})  # Primera columna más pequeña
    ...
    # Columna 2: Matriz de confusión
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=set(y), yticklabels=set(y), ax=ax[1])
    ax[1].set_title(f'Matriz de Confusión - {type(modelo).__name__}', fontsize=14, pad=10)
    ax[1].set_xlabel('Resultado', fontsize=12)
    ax[1].set_ylabel('Verdadero', fontsize=12)
    ax[1].tick_params(axis='both', labelsize=10)
